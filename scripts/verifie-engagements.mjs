...
/* LES ENGAGEMENTS, EPROUVES.

   Le juge vit dans src/shared/engagements.ts : ce qui est retenu, verse et
   reste, la decharge et la somme en lettres, et le terme de la piece
   d identite.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-engagements-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-engagements.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
