...
/* LE CALENDRIER PUR, EPROUVE.

   Le juge vit dans src/shared/agenda-pur.ts : le Trone, Ma Couronne et le
   site public lisent le meme calcul des heures libres.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-agenda-pur-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-agenda-pur.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
