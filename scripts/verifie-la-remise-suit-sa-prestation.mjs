...
/* LA REMISE SUIT SA PRESTATION —
   `node scripts/verifie-la-remise-suit-sa-prestation.mjs`.

   Une remise consentie a une cliente etait collee a la POSITION de la ligne,
   pas a la prestation : deplacer un geste la donnait au voisin, en silence.
   C'est de l'argent, et rien a l'ecran ne le disait. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-la-remise-suit-sa-prestation-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-la-remise-suit-sa-prestation.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
