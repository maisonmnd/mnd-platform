...
/* LES PARCOURS DE L'ACADEMIE, EPROUVES — `node scripts/verifie-parcours.mjs`.

   Poser deux fois les neuf parcours en ferait dix-huit, dont neuf jumeaux, et
   il faudrait les demeler un par un dans une liste qui porte deja des
   inscriptions. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-parcours-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-parcours.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
