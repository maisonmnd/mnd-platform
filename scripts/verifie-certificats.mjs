...
/* LE COFFRE DES CERTIFICATS, EPROUVE.

   Le juge vit dans src/shared/certificats-coffre.ts : le chemin d un
   certificat au coffre, le nom de son fichier, et ce que le coffre range.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-certificats-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-certificats.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
