...
/* LES CRENEAUX DE MA COURONNE, EPROUVES — `node scripts/verifie-creneaux.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-creneaux-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-creneaux.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
