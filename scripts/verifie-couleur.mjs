...
/* LE COMPTE D UNE CLIENTE, ÉPROUVÉ — `node scripts/verifie-couleur.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-couleur-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-couleur.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
