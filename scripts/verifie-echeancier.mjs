...
/* LE COMPTE D UNE CLIENTE, ÉPROUVÉ — `node scripts/verifie-echeancier.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-echeancier-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-echeancier.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
