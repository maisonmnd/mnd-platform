...
/* LES AVANCES DE L'EQUIPE, EPROUVEES — `node scripts/verifie-avances.mjs`.

   Une erreur ici ne plante pas : elle dit a la Maison qu'elle doit une somme
   qu'elle a deja rendue, ou qu'elle ne doit rien a qui a paye de sa poche. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-avances-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-avances.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
