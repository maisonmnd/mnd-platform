...
/* LA TRACE DE LA BASE, EPROUVEE.

   Le juge unique vit dans src/shared/traces.ts : l appareil lu, le
   geste nomme, les gestes sensibles, le resume par personne et les
   pieces liees a un rendez-vous. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-traces-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-traces.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
