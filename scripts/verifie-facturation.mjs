...
/* LE MOTEUR TARIFAIRE, ÉPROUVÉ — `node scripts/verifie-facturation.mjs`.

   On y garde surtout LE MUR du prix ferme convenu avec une cliente : il doit
   passer avant le calibre, le tarif au lock, le plancher et le Juste Prix.
   C'est de l'argent, et rien à l'écran ne dirait qu'il a été déplacé. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-facturation-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-facturation.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
