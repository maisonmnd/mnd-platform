...
/* LA FORMULE AU CALIBRE, EPROUVEE — `node scripts/verifie-calibre.mjs`.

   Une erreur ici ne plante pas : elle fait vendre a perte pendant dix mois, ou
   fait payer a une cliente le double de ce que la vitrine lui a promis. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-calibre-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-calibre.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
