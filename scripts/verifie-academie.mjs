...
/* L ACADEMIE, EPROUVEE.
   ...
   Le juge vit dans src/apps/trone/routes/equipe/academy.ts : quand les
   modules d une formation bougent, les seances et les notes suivent leur
   module par son nom.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-academie-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-academie.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
