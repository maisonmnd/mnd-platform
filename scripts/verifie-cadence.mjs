...
/* LA CADENCE D UNE TETE, EPROUVEE — `node scripts/verifie-cadence.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-cadence-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-cadence.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
