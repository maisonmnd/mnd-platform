...
/* LE COFFRE ET LES PRÊTS, ÉPROUVÉS — `node scripts/verifie-coffre.mjs`.

   Trois invariants : le coffre se retrouve toujours (fléché + non-fléché =
   le solde), un solde de prêt ne passe jamais sous zéro, et un objectif sans
   versement ne promet rien. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-coffre-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-coffre.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
