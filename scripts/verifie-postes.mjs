...
/* LES FICHES DE POSTE ET LE REGLEMENT, EPROUVES.

   Une fiche de poste sert a recruter, a evaluer et a defendre une sanction ; un
   reglement s'affiche et se remet contre decharge. Une rubrique absente ne se
   voit pas a l'ecran : elle se decouvre le jour d'un litige. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-postes-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-postes.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
