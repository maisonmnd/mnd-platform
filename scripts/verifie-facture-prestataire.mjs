...
/* LA FACTURE DU PRESTATAIRE, EPROUVEE.

   Le juge unique vit dans src/apps/trone/routes/equipe/facture.ts : les
   semaines du mardi au samedi, les prestations du Carnet, le total en
   lettres, et la paie qui attend la facture acceptee. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-facture-prestataire-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-facture-prestataire.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
