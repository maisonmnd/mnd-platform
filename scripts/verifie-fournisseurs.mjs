...
/* LES COMPTES FOURNISSEURS, EPROUVES — `node scripts/verifie-fournisseurs.mjs`.

   Une erreur ici ne plante pas : elle melange deux maisons, et l'on compare
   pendant des mois des chiffres devenus faux sans que rien ne le dise. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-fournisseurs-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-fournisseurs.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
