...
/* LA SAISIE D UNE DEPENSE, ALLEGEE, EPROUVEE.

   Le juge vit dans src/shared/depenses-saisie.ts : ce qui manque avant
   d enregistrer, le compte des trois reponses, la derniere depense d un
   beneficiaire, la liste jour par jour.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-depenses-saisie-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-depenses-saisie.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
