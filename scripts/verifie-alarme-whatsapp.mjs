...
/* L'ALARME DES MESSAGES SANS REPONSE, EPROUVEE.

   Le juge vit dans src/shared/conversations.ts (filsSansReponse), bati sur
   celui de la cloche. S'il se trompe, un message attend en silence et sa
   fenetre de 24 h se ferme sans que personne ne le voie.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-alarme-whatsapp-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-alarme-whatsapp.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
