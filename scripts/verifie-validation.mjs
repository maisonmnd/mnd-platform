...
/* LA VALIDATION DES DEPENSES, EPROUVEE — `node scripts/verifie-validation.mjs`.

   Une erreur ici ne plante pas : elle laisse entrer dans les comptes une depense
   que personne n'a regardee, ou fait disparaitre une depense reelle. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-validation-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-validation.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
