...
/* CONSULTATION D ABORD OU RESERVATION DIRECTE, EPROUVE.

   Le juge vit dans src/shared/qualification.ts : le site et Ma Couronne
   lisent la meme regle.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-qualification-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-qualification.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
