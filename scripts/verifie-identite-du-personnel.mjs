...
/* LA PIECE D'IDENTITE DU PERSONNEL, EPROUVEE.

   Le chemin vit dans src/shared/engagements-coffre.ts (cheminDeLaPiece).
   La base lit son deuxieme segment pour decider qui ouvre la piece : s'il
   se trompe, une carte d'identite s'ouvre a toute l'equipe.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-identite-du-personnel-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-identite-du-personnel.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
