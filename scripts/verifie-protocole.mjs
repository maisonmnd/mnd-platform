...
/* LES CRENEAUX DE MA COURONNE, EPROUVES — `node scripts/verifie-protocole.mjs`.

   Une erreur ici ne plante pas : elle fait venir une cliente pour une heure
   deja prise, ou lui fait trouver porte close. Cela se paie en confiance. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-protocole-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-protocole.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
