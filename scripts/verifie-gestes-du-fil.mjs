...
/* LES GESTES DU FIL, EPROUVES.

   Le juge vit dans src/shared/conversations.ts : la retenue avant le depart,
   qui peut reecrire un message parti, et les quatre pieges de la sonnette.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-gestes-du-fil-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-gestes-du-fil.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
