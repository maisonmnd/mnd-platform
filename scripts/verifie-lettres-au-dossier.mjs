...
/* LES LETTRES DU PRET RANGEES AU DOSSIER, EPROUVEES.

   Le chemin du coffre, les deux etats d'une lettre, et surtout : chaque
   phrase du PDF se retrouve mot pour mot dans la lettre a l'ecran.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-lettres-au-dossier-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-lettres-au-dossier.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
