...
/* LES CRENEAUX DE MA COURONNE, EPROUVES — `node scripts/verifie-serie.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-serie-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-serie.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
