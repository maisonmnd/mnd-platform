...
/* LES CARTES DE LA MAISON, EPROUVEES — `node scripts/verifie-cartes.mjs`.

   Une carte part CHEZ LA CLIENTE, en image, et ne se rattrape pas : un accord
   manque, une promesse que la Maison ne tient pas, un prenom vide au milieu du
   dessin, chacun se decouvre apres l'envoi. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-cartes-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-cartes.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
