...
/* LE REGLEMENT D UNE CONSULTATION, EPROUVE.

   Le juge vit dans src/shared/consultation-reglement.ts : le Trone ne dit
   regles que sur un montant pose par le serveur.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-consultation-reglement-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-consultation-reglement.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
