...
/* LA TOURNEE DU MATIN, EPROUVEE.

   Le juge vit dans src/shared/tournee.ts : ce que le journal des envois
   dit VRAIMENT d un rappel, et ses trois silences. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-tournee-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-tournee.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
