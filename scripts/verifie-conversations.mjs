...
/* LES SORTIES DE LA MAISON, EPROUVEES.

   Le juge unique vit dans src/shared/clients.ts : la lecture du carnet
   (Gbata sans Vekpe) et les cinq mois sans venir. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-conversations-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-conversations.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
