...
/* LES CALCULS DE SALON & FOYER — `node scripts/verifie-foyer.mjs`.

   Même montage que verifie-stock.mjs : le harnais (verifie-foyer.harnais.ts)
   importe la vraie couche de données, esbuild la bundle en simulant ce que le
   navigateur fournit. Un échec sort en code 1. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-foyer-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-foyer.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
