...
/* LES CODES DE PROMOTION, EPROUVES.

   Le juge unique vit dans src/shared/promos.ts : un code par cliente, a
   usage unique, 48 heures, honore a la caisse, sur le rendez-vous et dans
   Ma Couronne.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-promos-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-promos.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
