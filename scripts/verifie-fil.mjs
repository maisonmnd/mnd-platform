...
/* LE FIL ET LE TABLEAU — qui voit quoi, qui clôt quoi, qui déplace quoi — `node scripts/verifie-fil.mjs`.

   Même montage que verifie-stock.mjs : le harnais (verifie-fil.harnais.ts)
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-fil-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-fil.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
