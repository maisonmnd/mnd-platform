...
/* LE MOTEUR TARIFAIRE, ÉPROUVÉ — `node scripts/verifie-prix.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-prix-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-prix.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
