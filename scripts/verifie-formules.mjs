...
/* LE MOTEUR TARIFAIRE, ÉPROUVÉ — `node scripts/verifie-formules.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-formules-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-formules.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
