...
/* LES ENTREES HORS ACTIVITE, EPROUVEES.

   Le modele et ses juges vivent dans src/shared/finance.ts, la separation
   activite / hors activite dans src/shared/receipts.ts. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-hors-activite-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-hors-activite.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
