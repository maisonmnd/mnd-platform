...
/* LA DEVISE DE LA MAISON, ÉPROUVÉE — `node scripts/verifie-signature.mjs`.
...
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-signature-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-signature.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
