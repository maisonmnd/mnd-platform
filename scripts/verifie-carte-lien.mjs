...
/* LA SYNCHRO, EPROUVEE.
   ...
   Un rate reseau n'avait aucune reprise : la table restait rouge jusqu'au
   prochain geste. On juge ici le classement des messages en cause lisible, ce
   qui se retente, et l'espacement des reprises. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-carte-lien-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-carte-lien.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
