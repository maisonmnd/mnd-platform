...
/* LA PROVENANCE DES DÉPENSES, ÉPROUVÉE — .

   L'invariant qui compte : la Maison doit pouvoir dire DE QUI vient l'argent
   qu'elle a dépensé, ce qu'il reste de chaque revenu, et quand un revenu neuf
   a été entamé — sans qu'un ordre de lecture ne change la réponse. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-provenance-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-provenance.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
