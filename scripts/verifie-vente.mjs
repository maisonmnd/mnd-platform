...
/* CE QUI A ÉTÉ VENDU, ÉPROUVÉ — `node scripts/verifie-vente.mjs`.
   ...
   Le prix convenu et le contenu ajusté d'une tête. Une erreur ici ne plante
   pas : elle réclame au comptoir un prix que personne n'a accepté. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-vente-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-vente.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
