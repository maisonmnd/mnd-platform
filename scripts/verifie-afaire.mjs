...
/* LE TRAVAIL DE LA MAISON, EPROUVE — `node scripts/verifie-afaire.mjs`.

   Une erreur ici ne plante pas : elle envoie la Maison au mauvais travail, et
   l'on decouvre la faute apres une semaine passee dessus. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-afaire-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-afaire.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
