...
/* PLUSIEURS GESTES DANS UNE MEME VENUE, EPROUVE.

   Le juge vit dans src/apps/revelateur/agenda.ts : c'est lui qui range les
   familles par intention et qui porte le plafond de six. S'il se trompe, une
   visiteuse venue pour un entretien ne voit pas son lavage.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-plusieurs-gestes-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-plusieurs-gestes.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
