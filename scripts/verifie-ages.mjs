...
/* LES AGES QUI CONTREDISENT L HISTOIRE, EPROUVES.

   Le releve vit dans src/shared/ages.ts : ce que le carnet sait
   d une date de naissance qui ne peut pas etre vraie. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-ages-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-ages.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
