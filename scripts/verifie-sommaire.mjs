...
/* LE SOMMAIRE DES PARAMETRES, EPROUVE.

   Une famille ajoutee sans entree ne se trouve qu'en defilant ; une entree sans
   famille renvoie dans le vide. Les deux derivent en silence, au fil des
   reglages qu'on ajoute — c'est exactement ce qui est arrive entre le 13 aout
   et le 6 septembre. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-sommaire-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-sommaire.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
