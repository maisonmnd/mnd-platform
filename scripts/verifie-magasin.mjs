...
/* LE MAGASIN, EPROUVE.
   ...
   L'etat d'une reserve et le solde apres chaque mouvement vivent dans
   src/shared/stock.ts ; on les passe ici au banc. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-magasin-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-magasin.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
