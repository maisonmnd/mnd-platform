...
/* L'ARRIVEE D'UNE EMPLOYEE, EPROUVEE.

   Le juge vit dans src/shared/arrivee-pure.ts. Les memes regles sont ecrites
   en SQL dans supabase/migrations/0109_la_place_est_prete.sql : ce harnais
   garde les deux ecritures d'accord.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-l-arrivee-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-l-arrivee.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
