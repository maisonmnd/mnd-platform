...
/* LE CATALOGUE PAR PALIER, EPROUVE.

   Le juge vit dans src/shared/catalogue-par-palier.ts : le chiffre du
   palier, le seuil de la cliente, les longueurs ramenees a une ligne, les
   prestations groupees par marche.
 */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-catalogue-par-palier-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-catalogue-par-palier.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
