...
/* LA GAMME AU RENDEZ-VOUS, EPROUVEE — `node scripts/verifie-gamme.mjs`.

   Une erreur ici ne plante pas : elle fait payer a une cliente un prix qui
   n'est pas celui qu'on lui a annonce. Cela se paie en confiance. */

const racine = path.resolve(import.meta.dirname, '..');
const dossier = mkdtempSync(path.join(tmpdir(), 'verifie-gamme-'));
const sortie = path.join(dossier, 'harnais.mjs');

try {
  await build({
    entryPoints: [path.join(racine, 'scripts/verifie-gamme.harnais.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    outfile: sortie,
