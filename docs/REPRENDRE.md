...
ce qui l'entoure, au lieu d'apparier de memoire une balise entiere. Et sonder
SANS TENIR COMPTE DE LA CASSE, sur une chaine COURTE qu'un retour a la ligne
ne peut pas couper : un ecart de majuscules, ou une phrase qui enjambe deux
lignes, rendent le meme zero que les autres causes. Les deux sont arrivees
en verifiant cette parade-ci, l'une apres l'autre. Plus
...
- **Retour assumé sur 0099**, qui figeait le montant d'un devis retenu pour
  tous. Un prix qui CHANGE passe toujours au mieux par un avenant ; une faute
  de frappe se corrige.
- `pourquoiOnNeModifiePas` : retenu, direction seule ; à trancher, tout le
  personnel. `avertitAvantDeCorriger` dit le nouveau retenu et le nouveau reste
  AVANT d'enregistrer, ou le trop-versé si l'on descend sous les avances.
  `corrigeLeDevis` change le contenu, JAMAIS l'état ni le « retenu par » ;
  un retenu ne devient pas avenant par correction. Porte `corrigeLe` /
  `corrigePar`.
- Écran : « Corriger » sur chaque ligne de devis, formulaire prérempli (lignes
...
rituel vide) restent muets. LE VRAI TROU, trouvé une heure après
(« le RDV ne se charge pas pourtant j'ai refait honorer ») : le SÉLECTEUR DE
STATUT de la modale du RDV honorait SANS passer par honorAppointment — le
même chemin qui avait déjà oublié le stock (d'où « le stock suit le statut,
quel que soit le chemin »)… sans la reprise. Elle se tente désormais à
chaque sauvegarde « honoré » de la modale (import dynamique de actions,
pas de cycle à l'amorçage), idempotente par repriseDe, raison toastée.
Pour Befoune : recharger, rouvrir son rituel, Enregistrer avec le statut
honoré — la reprise se pose ou le toast nomme le blocage.
