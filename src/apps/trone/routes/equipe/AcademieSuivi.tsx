...
/* ---------- F6 · Jury ---------- */
/* ══ LA COPIE AU DOSSIER — 16 septembre 2026 ═══════════════════════════
   « Me permettre de sauvegarder le certificat » (Yéman). Le certificat
   enregistré depuis la page du certificat dépose sa copie au coffre des
   certificats, sous l'inscription ; elle se retrouve ici, et se rouvre par
   un lien signé d'une heure. Le coffre se relit quand la fenêtre reprend le
   focus : on revient de la page du certificat, la copie est là. */
function CopieAuDossier({ dossier }: { dossier: string }) {
  ...
  if (copies === null) return <div className="mnd-muted" style={{ fontSize: 11.5 }}>Le coffre se lit…</div>;
  if (copies.length === 0) {
    return (
      <div className="mnd-muted" style={{ fontSize: 11.5 }}>
        Aucune copie au dossier. Ouvrez le certificat, puis « Enregistrer le PDF » : la copie se dépose ici.
      </div>
    );
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
      <div className="tre-sec-label">Copie au dossier</div>
      {copies.map((c) => (
        <div key={c.chemin} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', borderTop: '1px solid var(--hairline)', paddingTop: 8 }}>
          ...
          <span className="mnd-muted" style={{ fontSize: 11.5 }}>déposée le {frDate(c.deposeLe)}</span>
...
function TabCertificat({ e, formation, modules, sc, mention }: { e: Enrollment; formation?: Formation; modules: string[]; sc: ReturnType<typeof scoreEnrollment>; mention: ReturnType<typeof mentionFor> }) {
  ...
  const link = (number: string, mn: 'certifie' | 'excellence', dateIso: string) => {
    const p = new URLSearchParams({
      ...
      /* L'INSCRIPTION : c'est là que « Enregistrer le PDF » dépose sa copie
         (16 septembre 2026, coffre des certificats). */
      dossier: e.id,
    });
    if (formation) {
